...
class Feed:
	def __init__(self):
		self.feed = feedparser.parse("http://pirateday.ath.cx/rss.xml")
...
def handler(req):
	...
	for i in feed.feed.entries:
		...
		if re.sub(r'(.*) \((.*)\)', r'\2', i.title.strip()) == "XXX":
			continue
		if re.sub(r'(.*) \((.*)\)', r'\2', i.title.strip()) == "XXX/Imageset":
			continue
		if re.sub(r'(.*) \((.*)\)', r'\2', i.title.strip()) == "XXX/HD":
			continue
		node.title = cgi.escape(re.sub(r'(.*) \((.*)\)', r'[\2] \1', i.title.strip()))
		node.description = cgi.escape(i.summary_detail.value)
		node.link = cgi.escape(i.link)
